...
async function generateACNLQR(newData) {
  ...
  switch (drawingTool.patternType) {
    case 0:
      path3D = injected.dress_long;
      break;
    case 1:
      path3D = injected.dress_half;
      break;
    case 2:
      path3D = injected.dress_none;
      break;
    case 3:
      path3D = injected.shirt_long;
      break;
    case 4:
      path3D = injected.shirt_half;
      break;
    case 5:
      path3D = injected.shirt_none;
      break;
  }
  if (path3D) {
  ...
  if (tInfo.sections instanceof Array) {
    //If we have a simple pattern type, take the custom width/height from it
  ...
  if (!path3D) {
    ...
    ctx.drawImage(
      ...
      threeCanvas.height,
      pattCenter - width / 2,
      (height - pattHeight) / 2 + threeCanvas.height,
      threeCanvas.width,
      threeCanvas.height
    );
  ...
  drawTxtWithBg(
    215 - drawingTool.title.length * 3,
    50 / 3,
    drawingTool.title,
    "#000000"
  );
